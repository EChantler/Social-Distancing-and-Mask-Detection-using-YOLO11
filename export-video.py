...
# create list of video paths and sources
video_paths = [
    ("5079568_Coworkers_Woman_1280x720.mp4", "Video content sourced from Freepik (https://www.freepik.com)"),
    ("5079566_Coworkers_Corridor_1280x720.mp4","Video content sourced from Freepik (https://www.freepik.com)"),
    ("5121314_School_Classroom_1280x720.mp4","Video content sourced from Freepik (https://www.freepik.com)"),
    ("sample_video.mp4","Video content sourced from Kaggle (https://www.kaggle.com/datasets/priteshraj10/detect-social-distancingcovid19)")
    ]
...
output_frames = []
for video_path, source in video_paths:
    tracker = Tracker(
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            for i in range(dist_pixel.shape[0]):
                for j in range(dist_pixel.shape[1]):  # Avoid duplicates
                    ...
                    if real_distance < 200:#if i in violations and j in violations:
                        ...
                        face_results = mask_model(face_frame, verbose=False)
            ...
            # Add the source text to the bottom left of the fram in small text
            cv2.putText(annotated_frame, source, (10, annotated_frame.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
...
print(f"Exporting video... Total frames: {len(output_frames)}")

for frame in output_frames:
    if frame.shape[1] != frame_width or frame.shape[0] != frame_height:
        print(f"Error: Frame dimensions do not match. Expected ({frame_width}, {frame_height}), got ({frame.shape[1]}, {frame.shape[0]})")
        continue
    out.write(frame)
